...
const skill_create = async(req, res) => {
    ...
    const skills = await new Skill(
        _.pick(...req.body, ["name", "level", "about"])
    );
...
const view_byId = async(req, res) => {
    ...
    try {
        ...
        return res
            .status(200)
            .send(_.pick(skills, ["name", "level", "about", "status"]));
    } catch (ex) {
        for (feild in ex.errors) res.status(400).json(ex.errors[feild].message);
    }
};
...
const view_all = async(req, res) => {
    try {
        ...
        if (req.admin.myskills == "")
            return res.status(200).send("You dosen't have any skills");

        res
            .status(200)
            .send(
                _.pick(...req.admin.myskills, ["name", "level", "about", "status"])
            );
    } catch (ex) {
        console.log(ex);
        for (feild in ex.errors) res.status(400).json(ex.errors[feild].message);
