    ...
    try {
        bcrypt.hash(req.body.password, 10, async(err, hash) => {
            if (err) {
                ...
                if (user)
                    return res.status(409).send({ msg: "user is already created." });
...
const user_login = async(req, res) => {
    ...
    try {
        ...
        res
            .status(200)
            .send(
                _.pick(user, ["name", "email", "address", "phone", "gender", "status"])
            );
    } catch (err) {
        for (field in err.occur) res.status(400).json(err.occur[field].message);
    }
...
const delete_myProfile = async(req, res) => {
    try {
        ...
        if (!req.user.type === "admin")
            return res.status(400).send("Unable to delete an admin Account!!..");

        await req.user
            .remove()
            .then(() =>
                res.status(200).send("Your accout has been deleted successfully")
            )
